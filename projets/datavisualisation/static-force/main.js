...
fetch(`dataset.json`)
  ...
  .then(ordoredDataSetByDate => {
    ...
    let data = ordoredDataSetByDate.filter(node => node.date.includes("/10/18")).map(item => {
      return {
        value: item.date,
    ...
    function updateDropDownFilter(dates) {
      const select = document.querySelector("#select");
      // remove childs
      select.childNodes.forEach(node => select.removeChild(node));

      for (let label of new Set(Array.from(dates.keys()).map(date => {
        const parts = date.split("/");
        return `${parts[0]}/${parts[1]}`;
      })).values()) {
        const option = document.createElement("option");
        option.textContent = moment(label).format('MMM YYYY');
        option.value = label
        select.appendChild(option);
      }
    }

    updateDropDownFilter(mappedData);
    ...
    // const dataX = Array.from(mappedData.keys())
